/*
 * Created by S.Dobranos on 19.11.20 14:53
 * Copyright (c) 2020. All rights reserved.
 */

package com.fromfinalform.tetris.domain.model.game.classic
...
import com.fromfinalform.tetris.data.model.game.classic.ClassicGameResults
...
import com.fromfinalform.tetris.domain.interactor.IGameField
...
class ClassicGameLooper : IGameLooper {
    ...
    private var currentFigureHoldedHandler: ((f: IFigure, x: Float, y: Float, dx: Float, dy: Float, hardDropped: Boolean) -> Unit)? = null
    override fun withOnCurrentFigureHolded(handler: (f: IFigure, x: Float, y: Float, dx: Float, dy: Float, hardDropped: Boolean) -> Unit): IGameLooper {
            ...
            if (holdDesiredTimeMs < 0) {
                ...
                if (timeMs >= holdDesiredTimeMs) {
                    ...
                    if (field.hold(currentFigure!!)) {
                        currentFigureHoldedHandler?.invoke(
                            currentFigure!!,
                            currentFigure!!.x,
                            currentFigure!!.y, currentFigure!!.x - xPrev,
                            currentFigure!!.y - yPrev,
                            figureHardDroppedAtY > Byte.MIN_VALUE.toFloat())
